...
export function D1Adapter(db) {
	...
	return {
		...
		async getUser(id) {
			return await getRecord(await resolvedDb, GET_USER_BY_ID_SQL, [id]);
		},
		async getUserByEmail(email) {
			return await getRecord(await resolvedDb, GET_USER_BY_EMAIL_SQL, [email]);
		},
		async getUserByAccount({ providerAccountId, provider }) {
			return await getRecord(await resolvedDb, GET_USER_BY_ACCOUNTL_SQL, [
				providerAccountId,
				provider
			]);
		},
		async updateUser(user) {
			const params = await getRecord(await resolvedDb, GET_USER_BY_ID_SQL, [user.id]);
			if (params) {
				...
				const res = await updateRecord(await resolvedDb, UPDATE_USER_BY_ID_SQL, [
					params.name,
					params.email,
				...
				if (res.success) {
					...
					const user = await getRecord(await resolvedDb, GET_USER_BY_ID_SQL, [params.id]);
					if (user) return user;
		...
		async deleteUser(userId) {
			...
			await deleteRecord(await resolvedDb, DELETE_ACCOUNT_BY_USER_ID_SQL, [userId]);
			await deleteRecord(await resolvedDb, DELETE_SESSION_BY_USER_ID_SQL, [userId]);
		...
		async unlinkAccount({ providerAccountId, provider }) {
			await deleteRecord(await resolvedDb, DELETE_ACCOUNT_BY_PROVIDER_AND_PROVIDER_ACCOUNT_ID_SQL, [
				provider,
				providerAccountId
			]);
		},
		async createSession({ sessionToken, userId, expires }) {
			console.info("LOG", "Create session");
		...
		async getSessionAndUser(sessionToken) {
			console.info("LOG", "Get Session and User");

			const session = await getRecord(await resolvedDb, GET_SESSION_BY_TOKEN_SQL, [sessionToken]);
			// no session?  no user!
			if (session === null) return null;
			// this shouldnt happen, but just in case
			const user = await getRecord(await resolvedDb, GET_USER_BY_ID_SQL, [session.userId]);
			if (user === null) return null;
			return { session, user };
		},
		async updateSession({ sessionToken, expires }) {
			console.info("LOG", "Update Session");
			...
			await updateRecord(await resolvedDb, UPDATE_SESSION_BY_SESSION_TOKEN_SQL, [
				expires?.toISOString(),
				sessionToken
			]);
			return await db
		...
		async deleteSession(sessionToken) {
			console.info("LOG", "Delete Session");
		...
		async useVerificationToken({ identifier, token }) {
			...
			await deleteRecord(await resolvedDb, DELETE_VERIFICATION_TOKEN_SQL, [identifier, token]);
