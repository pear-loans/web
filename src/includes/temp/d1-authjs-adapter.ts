...
export const CREATE_USER_SQL =
	"INSERT INTO authjs_users (id, name, email, emailVerified, image) VALUES (?, ?, ?, ?, ?)";
export const GET_USER_BY_ID_SQL = "SELECT * FROM authjs_users WHERE id = ?";
export const GET_USER_BY_EMAIL_SQL =
	"SELECT * FROM authjs_users WHERE email = ?";
...
export const CREATE_SESSION_SQL =
	"INSERT INTO authjs_sessions (id, sessionToken, userId, expires) VALUES (?,?,?,?)";
export const GET_SESSION_BY_TOKEN_SQL = `
  SELECT id, sessionToken, userId, expires
  FROM authjs_sessions
  WHERE sessionToken = ?`;
export const UPDATE_SESSION_BY_SESSION_TOKEN_SQL =
	"UPDATE authjs_sessions SET expires = ? WHERE sessionToken = ?";
export const DELETE_SESSION_SQL =
	"DELETE FROM authjs_sessions WHERE sessionToken = ?";
export const DELETE_SESSION_BY_USER_ID_SQL =
	"DELETE FROM authjs_sessions WHERE userId = ?";
...
export const CREATE_ACCOUNT_SQL = `
  ...
  VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)`;
export const GET_ACCOUNT_BY_ID_SQL =
	"SELECT * FROM authjs_accounts WHERE id = ? ";
export const GET_ACCOUNT_BY_PROVIDER_AND_PROVIDER_ACCOUNT_ID_SQL =
	"SELECT * FROM authjs_accounts WHERE provider = ? AND providerAccountId = ?";
export const DELETE_ACCOUNT_BY_PROVIDER_AND_PROVIDER_ACCOUNT_ID_SQL =
	"DELETE FROM authjs_accounts WHERE provider = ? AND providerAccountId = ?";
export const DELETE_ACCOUNT_BY_USER_ID_SQL =
	"DELETE FROM authjs_accounts WHERE userId = ?";

// VERIFICATION_TOKEN
export const GET_VERIFICATION_TOKEN_BY_IDENTIFIER_AND_TOKEN_SQL =
	"SELECT * FROM authjs_verification_tokens WHERE identifier = ? AND token = ?";
export const CREATE_VERIFICATION_TOKEN_SQL =
	"INSERT INTO authjs_verification_tokens (identifier, expires, token) VALUES (?,?,?)";
export const DELETE_VERIFICATION_TOKEN_SQL =
	"DELETE FROM authjs_verification_tokens WHERE identifier = ? and token = ?";
...
export async function createRecord<RecordType>(
	...
	try {
		...
		console.log(
			`Error creating record with ${CREATE_SQL} and bindings ${bindings}`,
		);
...
export function D1Adapter(db: BothDB): Adapter {
	const resolvedDb = Promise.resolve(db);
	return {
		...
		async updateUser(user) {
			const params = await getRecord<AdapterUser>(
				await resolvedDb,
				GET_USER_BY_ID_SQL,
				[user.id],
			);
			if (params) {
				...
				if (res.success) {
					...
					const user = await getRecord<AdapterUser>(
						await resolvedDb,
						GET_USER_BY_ID_SQL,
						[params.id],
					);
					if (user) return user;
					throw new Error(
						`couldnt find user after update with id ${params.id}`,
					);
				}
				throw new Error(
					`couldnt update user with data ${JSON.stringify(user)}`,
				);
			}
		...
		async deleteUser(userId) {
			...
			await deleteRecord(await resolvedDb, DELETE_ACCOUNT_BY_USER_ID_SQL, [
				userId,
			]);
			await deleteRecord(await resolvedDb, DELETE_SESSION_BY_USER_ID_SQL, [
				userId,
			]);
			await deleteRecord(await resolvedDb, DELETE_USER_SQL, [userId]);
			return null;
		},
		async linkAccount(a) {
		...
		async updateSession({ sessionToken, expires }) {
			...
			if (expires === undefined) {
				await deleteRecord(await resolvedDb, DELETE_SESSION_SQL, [
					sessionToken,
				]);
				return null;
			}
		...
		async deleteSession(sessionToken) {
			await deleteRecord(await resolvedDb, DELETE_SESSION_SQL, [sessionToken]);
			return null;
		},
		...
		async useVerificationToken({ identifier, token }) {
			...
			await deleteRecord(await resolvedDb, DELETE_VERIFICATION_TOKEN_SQL, [
				identifier,
				token,
			]);
